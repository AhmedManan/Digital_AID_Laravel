...
<form action="/admin/reports" method="post">
  ...
  @if($result->status=="unsolved")
    <tr>
      ...
      <td><a href="/admin/viewprofile/{{$result->sender}}">{{$result->sender}}</td>
      <td>
      ...
      <td><div style="padding-bottom: 18px;"><input class="btn btn-info" name="Submit" value="Respond" type="submit"/></div></td>
    </tr>
    @else
    <tr>
      <td scope="row">{{$result->id}}</th>
      ...
      <td><div style="padding-bottom: 18px;">Responded by: <span style="color: green;"><a href="/admin/viewprofile/{{$result->actioned_by}}">{{$result->actioned_by}}</span><br/> </div></td>
    </tr>
    @endif
    @endforeach
  </tbody>
